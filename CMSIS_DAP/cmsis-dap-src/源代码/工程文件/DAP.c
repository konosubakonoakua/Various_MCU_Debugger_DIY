...
uint8_t  Watch_registers [DAP_PACKET_SIZE];  
uint8_t  Watch_data [DAP_PACKET_SIZE];  
uint32_t write_data;
extern void LedRunningOn(void)	;
extern void LedRunningOff(void)	;
...
//	u8 data_count= 0;
	...
	while (request_count--)
	{	
//		Watch_data[data_count*5] = request_value = *request++;
...
//			Watch_data[data_count*5 +1] = *(request+0) <<  0;
//			Watch_data[data_count*5 +2] = *(request+1) <<  8;
//			Watch_data[data_count*5 +3] =	*(request+2) << 16;
//			Watch_data[data_count*5 +4] =	*(request+3) << 24;
//			data_count ++;
...
static uint32_t DAP_SWD_TransferBlock(uint8_t *request, uint8_t *response)
{
	...
	request_count = *request | (*(request+1) << 8); //16位计数值
	request += 2;
	if (request_count == 0) goto end;

	request_value = *request++;  //包含读写以及地址信息
	if (request_value & DAP_TRANSFER_RnW)
	{
		// Read register block
		if (request_value & DAP_TRANSFER_APnDP)
	...
	*response++ = *request;
	...
	switch (*request++)
	{
		...
		case ID_DAP_SWJ_Pins:
		   ...
		   //进行软件复位
//			write_data = 0xe000ed0c;SWD_Transfer(0x05,&write_data);Delayms(1);// SCB->AIRCR
//			write_data = 0x05fa0004;SWD_Transfer(0x0d,&write_data);Delayms(1);		
		
//			if(SWD_Transfer(0x09,&write_data) == DAP_TRANSFER_OK);
//				{			
//					LedRunningOn();
//					Delayms(20);
//					LedRunningOff();
